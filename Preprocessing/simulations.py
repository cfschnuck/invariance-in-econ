...
d_f = f_factors[:,0]**2 - torch.sin(f_factors[:,1]) + torch.log(f_factors[:,2] + 1)
d_x = x_factors[:,0]**2 + 4 * x_factors[:,1] + torch.sin(x_factors[:,2]) + x_factors[:,3] + torch.log(x_factors[:,4] + 1)

d = d_x + d_f

y = 6 * d + 3 * x_factors[:,1] - torch.sin(x_factors[:,2]) + torch.log(x_factors[:,0] + 1)
